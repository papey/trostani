...
async function handleStart(origin: Message, client: Challonge, config: any) {
  ...
  (origin.channel as TextChannel).send(
    `Random rolling tournament **${tnmt["name"]}**`,
  );
  ...
  (origin.channel as TextChannel).send(
    `Tournament **${tnmt["name"]}** started ! 🃏 GLHF 🃏 !`,
  );
}
...
async function handleDecks(origin: Message, builder: any) {
  ...
  if (results.length == 0) {
    (origin.channel as TextChannel).send(`No decks found for this tournament`);
    return;
  }
...
async function handleStatus(
  ...
  (origin.channel as TextChannel).send(resp);
}
...
async function handleJoin(
  ...
  (origin.channel as TextChannel).send(`_Processing ${displayName} decklist_`);
...
async function handleList(cmd: Command, origin: Message, client: Challonge) {
  ...
  if (parts.length == 0) {
    (origin.channel as TextChannel).send(
      `Sorry, there is no entry matching the request`,
    );
  } else {
    // push message to channel by joining outputs parts
    (origin.channel as TextChannel).send(parts.join("\n"));
  }
}
